...
const Checkout = ({ items, onSubmit }: any) => {
    ...
    return <div className="select-none h-auto w-full text-center pt-3 pb-4 px-4">
        <div className="flex mb-3 text-lg font-semibold text-blue-gray-700">
            <div>TOTAL</div>
            <div className="text-right w-full">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(total)}</div>
        </div>
        ...
        <button onClick={() => {
            onSubmit(cash);
        }} className="text-white rounded-2xl text-lg w-full py-3 focus:outline-none bg-blue-500 hover:bg-blue-600">
            SUBMIT
        </button>
    </div>;
};
